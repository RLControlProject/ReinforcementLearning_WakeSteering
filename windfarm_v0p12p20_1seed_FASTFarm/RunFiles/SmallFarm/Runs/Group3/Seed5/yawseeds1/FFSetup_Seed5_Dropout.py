...
RUNTYPE = "rl" # turbsim, baseline or rl
num_turbines = 3
...
def FFSetup(outpath, wind_spd, wind_dir, shear, TI, yaw_angles,FFomp):
    ...
    # ----------- Case absolute path
    btsdir = os.path.join(thisdir,'FFSim_Seed5')

    # ----------- Additional variables
    if num_turbines == 3:
        tmax = 700  # Total simulation time
    ...
    dropoutidx = [1] # Dropout the second turbine
    for cond in range(case.nConditions):
        for casei in range(case.nCases):
            ...
            for t in range(case.nTurbines):
                ...
                elastodynfile = os.path.join(casepath,EDfilename+f"{t+1}_mod.dat")
                with open(elastodynfile,'r') as f:
                    elastodynlines = f.readlines()
                if t in dropoutidx:
                      servodynlines[25] = "0    " + servodynlines[25][7:]
                      servodynlines[8] = "0    " + servodynlines[8][7:]
                      servodynlines[9] = "0    " + servodynlines[9][7:]
                      servodynlines[10] = "0    " + servodynlines[10][7:]
                      servodynlines[14] = "90   " + servodynlines[14][7:]
                      servodynlines[15] = "90   " + servodynlines[15][7:]
                      servodynlines[16] = "90   " + servodynlines[16][7:]
                      elastodynlines[27] = "90   " + elastodynlines[27][7:]
                      elastodynlines[28] = "90   " + elastodynlines[28][7:]
                      elastodynlines[29] = "90   " + elastodynlines[29][7:]
                      elastodynlines[32] = "0    " + elastodynlines[32][7:]
                with open(servodynfile,'w') as f:
                    f.writelines(servodynlines)
                with open(elastodynfile,'w') as f:
                    f.writelines(elastodynlines)
...
if __name__ == "__main__":
    FFSetup(
        outpath=os.path.join(thisdir, "./FFSim_Seed5"),
        wind_spd=10.0,
