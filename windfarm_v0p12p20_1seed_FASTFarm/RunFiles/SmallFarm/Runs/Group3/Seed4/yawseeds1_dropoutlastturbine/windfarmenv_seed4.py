...
from FFSetup_Seed4_Dropout import FFSetup as FFSetup_Dropout
...
class FFProblem:
    def __init__(self,simseed,layout_grid,num_turbines,FFomp,episodenum):
    ...
    def _RunProb(self):
        if self.episodenum <= 10:           # (use numiteration  / numenv)
            print(f"Running without dropout for episodenum = {self.episodenum}")
            ...
            print(f"Running with dropout for episodenum = {self.episodenum}")
            self.FFSetupSeed = FFSetup_Dropout(outpath=self.outpath,wind_spd = self.wind_spd,\
                    wind_dir=self.wind_dir,shear=self.shear,TI=self.TI,yaw_angles = self.yaw_angles,FFomp = self.FFomp)
...
class WindFarmEnv_v0p12(gym.Env):
    ...
    def __init__(self, windtype:'DiscreteRandom', initialyaw:'ZeroYaw', algorithm:'SAC',FFomp:False):
        ...
        with open('log.txt','w') as f:
            f.write("simseed,sum_turbine_powers,floris_opt,rew,rewbaseline,wdir,ws,TI,FFSeed\n")
            f.flush()
    ...
    def get_turbine_powers(self, u:np.ndarray):
        ...
        self.FFProb = FFProblem(
            simseed=simseed,
            layout_grid=layout_grid,
            num_turbines=num_turbines,
            FFomp=self.FFomp,
            episodenum=self.episode_step_count,
        )
        # Set Inflow for FF
