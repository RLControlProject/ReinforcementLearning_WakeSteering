...
from FFSetup_Seed6 import FFSetup
...
class FFProblem:
    def __init__(self,simseed,layout_grid,num_turbines,FFomp):
        ...
        self.outpath = os.path.join('FFSim_Seed6_RL','Sim_'+str(simseed))
        self.FFomp = FFomp
    ...
    def GetPwr(self):
        self._RunProb()
        allpowers,DEL = self._ReadPwr()
        deletetest = np.random.randint(0,100)
        # if deletetest != 2:
        #     self._DeleteSim()
        self._Deletebtsout()
        return allpowers, self.FFSetupSeed, DEL # To take into account the order of power in FF vs RL code
    ...
    def _ReadPwr(self):
        ...
        for i in range(num_turbines):
            ...
            t = Time.iloc[-1]-tinit
            for var,slope in SNslopes.items():
                ranges = fatpack.find_rainflow_ranges(dfi[var][Time > tinit])
                Nrf, Srf = fatpack.find_range_count(ranges,binnum_del)
                DELpart = Srf**slope * Nrf / t
                DEL[var] = DEL[var] + DELpart.sum() ** (1/slope)
        for var in SNslopes:
            DEL[var] = DEL[var]/num_turbines
        return allpowers,DEL
...
class WindFarmEnv_v0p12(gym.Env):
    ...
    def __init__(self, windtype:'DiscreteRandom', initialyaw:'ZeroYaw', algorithm:'SAC',FFomp:False,rewardtype:'base'):
    ...
    def get_turbine_powers(self, u:np.ndarray):
        ...
        # Create FF problem
        simseed=random.randint(0, 10000000000000)
        ...
        # Get power from FF
        turbine_powers,FFSeed,DEL = self.FFProb.GetPwr()
        self.FFSeed=FFSeed

        return turbine_powers, yaw_angles, yaw_angles_actual, FFSeed, simseed, DEL
    ...
    def step(self, u:np.ndarray)->tuple[np.ndarray, float, bool, dict]:
        ...
        turbine_powers, yaw_angles, yaw_angles_actual, FFSeed, simseed, DEL = self.get_turbine_powers(u)
        with open(os.path.join(THIS_FILE_PATH,'optpower_1seeddata',f'TI{self.wind.state[2]}',f'wdir{self.wind.state[0]}',f'FFPower_FlorisOptimal_seed{self.FFSeed}.pkl'),'rb') as f:
            optpower_1seed=pickle.load(f)
        self.floris_opt_stochastic_1seed = np.flip(np.array(optpower_1seed).tolist())
    ...
    def reset(self, seed:int=None):
        ...
        if self.windtype=='DiscreteRandom': # (FLORIS results are loaded from "setup" folder based on wind state)
            with open('FFPower_FlorisOptimal_Seed6.pkl','rb') as f:
            ...
            self.floris_opt = np.flip(np.array(df_cond['optpower'].tolist()[0]))
