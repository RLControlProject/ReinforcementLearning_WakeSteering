...
class WindFarmEnv_v1p3(gym.Env):
    ...
    def reset(self, seed:int=None):
        ...
        if self.windtype=='DiscreteRandom': # (FLORIS results are loaded from "setup" folder based on wind state)
            ...
            self.floris_opt_stochastic_1seed = np.nan # there is no stochasticism in this case, so this is meaningless (i.e., ) self.floris_opt is already the true answer
