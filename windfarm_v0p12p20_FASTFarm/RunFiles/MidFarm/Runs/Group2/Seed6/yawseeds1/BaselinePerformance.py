...
outpath = os.path.join(THISDIR, "FFSim_Baseline_Seed6")
runsim = True
        ...
        for wind_spd in wind_spd_all:
            for wind_dir in wind_dir_all:
                for turb_int in [TI]:
                    ...
                    yaw_np_opt = np.array(yaw_angles_opt_ordered).reshape(1, 1, num_turbines)
                    yaw_np_static_offset = np.array(yaw_angles_static_offset_ordered).reshape(1, 1, num_turbines)
                    ...
                    allseedpower = []
                    turbinepowers_allseeds = np.zeros(num_turbines)
                    for seed in range(10):
                        ...
                        for turb in range(num_turbines):
                            ...
                            turbinepowers.append(Poweri)
                            turbinepowers_allseeds[turb] = turbinepowers_allseeds[turb] + Poweri
                            seedpower = seedpower + Poweri
                        allseedpower.append(seedpower)
                        optpower_1seed_dir = os.path.join(THISDIR,'optpower_1seeddata',f'TI{turb_int}',f'wdir{wind_dir}')
                        os.makedirs(optpower_1seed_dir,exist_ok=True)
                        with open(os.path.join(optpower_1seed_dir,f'FFPower_FlorisOptimal_seed{seed}.pkl'),'wb') as f:
                            optpower_1seed = list(turbinepowers)
                            pkl.dump(optpower_1seed,f)
                    turbinepowers_allseeds = np.array(turbinepowers_allseeds) / 10
                    df_FFPower_FlorisOptimal.loc[len(df_FFPower_FlorisOptimal)] = [
                        ...
                        turbinepowers_allseeds,
                        allseedpower
                    ]
    with open(f"FFPower_FlorisOptimal_Seed6_{TI}.pkl", "wb") as f:
