...
"""how to run:
cd /ascldap/users/kbrown1/tscratch/Reinforcement_Learning/RLControl/windfarm_v0p12p20
conda activate RLControlEnv
python3 sac_post_generate_sequence_5_100env_medium_3.py
"""
...
from windfarm_env_inputindices_medium_3 import WindFarmEnv_v1p3, tag, num_timesteps, num_turbines, myseeds
...
if __name__ == "__main__":
    ...
    # set up log dir
    tb_log_name = 'trial_5_100env'+tag
    ...
    env_kwargs['rewardtype'] = 'stochastic' # 'base' 'stochastic' 'stochastic_1seed'
    ...
    if trainModel:
        ...
        n_envs = 100
        venv0 = make_vec_env(WindFarmEnv_v1p3,vec_env_cls=SubprocVecEnv, n_envs=n_envs, seed=myseeds[0], env_kwargs=env_kwargs)
        ...
        tb_log_name_override = 'trial_5_10env'+tag # for simplicity, reverting to hyperparameters from 10 env, which gives a conservative estimate of peak performance using 100env instead of 10env
        log_dir_optuna = os.path.join(os.path.dirname(__file__))+"/sac_optuna_search_sequence/trials"
        summary_dir = os.path.join(log_dir_optuna,tb_log_name_override[:-1]+'1_1','summary')
    ...
    if testModel: # evaluate the policy (source: https://colab.research.google.com/github/Stable-Baselines-Team/rl-colab-notebooks/blob/sb3/advanced_saving_loading.ipynb#scrollTo=T65Bo7-k3dWL)
        ...
        if testModel_generateData:
            # set run-specific environment args
            env_kwargs['windtype'] = 'ContinuousRandom'
            ...
            print("Calculated learning_rate:", model.learning_rate)
            print("Calculated learning_starts:", model.learning_starts)
            print("Calculated train_freq:", model.train_freq)
            print("Calculated tau:", model.tau)
            print("Calculated target_update_interval:", model.target_update_interval)
            print("Calculated ent_coef:", model.ent_coef)
            print("Calculated target_entropy:", model.target_entropy)
