...
python3 sac_post_generate_sequence_4_10env_extralarge_moremismatch.py
"""
...
from windfarm_env_inputindices_extralarge_moremismatch import WindFarmEnv_v1p3, tag, num_timesteps
...
if __name__ == "__main__":
    ...
    if trainModel:
        ...
        if not load_pretrained_model:
            ...
            model = SAC( 
                ...
                ent_coef = 'auto_'+str(0.0743353182496901),
                batch_size = 25000,
                use_sde = False,
                gamma = 0,
    ...
    if testModel: # evaluate the policy (source: https://colab.research.google.com/github/Stable-Baselines-Team/rl-colab-notebooks/blob/sb3/advanced_saving_loading.ipynb#scrollTo=T65Bo7-k3dWL)
        ...
        if testModel_generateData:
            ...
            n_envs = 100
            venv0 = make_vec_env(WindFarmEnv_v1p3,vec_env_cls=SubprocVecEnv, n_envs=n_envs, seed=0, env_kwargs=env_kwargs)
            venv = VecMonitor(venv=venv0, filename=os.path.join(log_dir,tb_log_name+'_1'), info_keywords=(["r_stochastic", "power", "power_floris_opt_stochastic", "power_floris_opt", "yaw_angles", "yaw_angles_floris_opt", "rel_yaw_angles", "rel_yaw_angles_floris_opt", "wind_dir", "wind_spd", "turb_int"]))
