...
python3 sac_optuna_search_sequence_5_10env_extralarge_1.py > sac_optuna_search_sequence/trials/trial_5_10env_extralarge_1_1/output.txt
...
from windfarm_env_inputindices_extralarge_1 import WindFarmEnv_v1p3, tag, num_timesteps, num_turbines
...
def train_model(trial, log_dir = '', tb_log_name = ''):
    ...
    learning_rate = trial.suggest_loguniform("learning_rate", 1e-6, 1e-3) # original: 1e-6
    learning_starts = 0#trial.suggest_int("learning_starts", 0,100) 
    ...
    train_freq = trial.suggest_int("train_freq", 1,2*n_envs+1, step = n_envs)
    ent_coef = trial.suggest_loguniform("ent_coef", 0.001, 0.9) # original: 0.2
    target_entropy_multiplier = trial.suggest_int("target_entropy_multiplier", 1,1)
    ...
    # target_update_interval = trial.suggest_int("target_update_interval", 1,3*n_envs+1, step = n_envs)
